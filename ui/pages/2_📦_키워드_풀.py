...
st.title("📦 ② 키워드 풀 관리")
st.caption("저장된 키워드를 한 곳에서 정리하세요. 점수 낮은 키워드는 자동으로 정리할 수 있어요.")


def _refresh():
    # data_editor 위젯 키 초기화로 즉시 반영
    ...
    with st.container(border=True):
        ...
        with col_l:
            st.markdown(
                ...
                f"`{os.path.basename(xlsx_path)}` (아래 버튼으로 내려받아 Excel/Numbers에서 직접 편집 → "
                f"같은 파일을 다시 저장만 하면 앱이 자동으로 인식)"
                f"</span>",
...
# ── 양방향 동기화 패널 (mirror 백엔드일 때만) ────────────
if is_mirror_backend():
    status = sync_status() or {}
    with st.container(border=True):
        col_a, col_b, col_c, col_d = st.columns([1.6, 1.6, 1.4, 1.6])
        with col_a:
            on = "🟢 켜짐" if status.get("sheets_enabled") else "🔴 미연결"
            st.metric("Google Sheets 연동", on)
        with col_b:
            pending = status.get("pending_writes", 0)
            label = f"보류 변경 {pending}건" if pending else "동기화 ✓"
            st.metric("쓰기 대기열", label)
        with col_c:
            last_pull = status.get("last_pull_at") or "—"
            st.metric("마지막 새로고침", last_pull[-8:] if last_pull != "—" else "—",
                      help=last_pull)
        with col_d:
            last_push = status.get("last_push_at") or "—"
            st.metric("마지막 업로드", last_push[-8:] if last_push != "—" else "—",
                      help=last_push)

        b1, b2, b3 = st.columns([1, 1, 2])
        with b1:
            if st.button("☁️  시트에서 새로고침", use_container_width=True,
                         help="구글 시트에서 직접 수정한 변경사항을 가져옵니다."):
                with st.spinner("구글 시트에서 가져오는 중…"):
                    res = pull_from_sheets()
                if res.get("ok"):
                    st.success(
                        f"가져오기 완료 — 키워드 {res['pool_count']}개, "
                        f"글 {res['content_count']}건 ({res['elapsed']}초)"
                    )
                    log_activity("success", "동기화", "Sheets → 로컬 pull",
                                 f"pool={res['pool_count']} content={res['content_count']}")
                    _refresh()
                else:
                    st.error(f"가져오기 실패: {res.get('reason', '알 수 없음')}")
                    log_activity("error", "동기화", "Sheets pull 실패", str(res))
        with b2:
            if st.button("📤  시트로 업로드", use_container_width=True,
                         help="현재 로컬에 있는 모든 키워드를 구글 시트로 일괄 업로드합니다."):
                with st.spinner("구글 시트에 업로드 중…"):
                    res = push_to_sheets()
                if res.get("ok"):
                    st.success(
                        f"업로드 완료 — 키워드 {res['pool_count']}개, "
                        f"글 {res['content_count']}건 ({res['elapsed']}초)"
                    )
                    log_activity("success", "동기화", "로컬 → Sheets push",
                                 f"pool={res['pool_count']} content={res['content_count']}")
                else:
                    st.error(f"업로드 실패: {res.get('reason', '알 수 없음')}")
                    log_activity("error", "동기화", "Sheets push 실패", str(res))
        with b3:
            if status.get("sheet_url"):
                st.markdown(
                    f"<div style='padding-top:6px'>🔗 <a href='{status['sheet_url']}' "
                    f"target='_blank'>구글 시트 열기</a></div>",
                    unsafe_allow_html=True,
                )
            elif status.get("last_error"):
                st.caption(f"⚠️ {status['last_error']}")
...
with top_l:
    ...
    st.caption(
        "‘유효기간이 지난 키워드’를 자동으로 재평가/삭제하고, 풀 크기를 최대 한도로 맞춥니다."
    )
with top_r:
    if st.button("🧹  지금 정리 실행", use_container_width=True):
        try:
            ...
            st.success(
                f"정리 완료 — 재평가 {report.revaluated} / 유지 {report.refreshed} / "
                f"만료 {report.expired} / 변형추가 {report.transmuted} / "
                f"초과제거 {report.overflow_removed}"
            )
            log_activity("success", "풀 정리", f"prune 실행", report.summary())
            _refresh()
        except Exception as e:
...
# 필터 적용
view = df.copy() if hasattr(df, "copy") else list(df)
if hasattr(view, "query"):  # pandas
    ...
    sort_map = {
        "점수(높은순)": ("score", False),
        "점수(낮은순)": ("score", True),
        "검색량(높은순)": ("search_volume", False),
        "최근 평가일순": ("updated_at", False),
    }
    col, asc = sort_map[sort_by]
    view = view.sort_values(by=col, ascending=asc)

# ── 표시(편집 가능 — 체크박스로 선택 후 일괄 액션) ────────
...
with b1:
    delete_clicked = st.button(
        f"🗑  선택 삭제 ({len(selected_ids)})",
        disabled=len(selected_ids) == 0,
        use_container_width=True,
    )
with b2:
    revaluate_clicked = st.button(
        f"🔄  선택 재평가 ({len(selected_ids)})",
        disabled=len(selected_ids) == 0,
        use_container_width=True,
    )
