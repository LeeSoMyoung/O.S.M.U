"""기술 용어를 사용자 친화 한국어 메시지로 번역.

요구사항 §7:
    ❌ "PermissionError: seed cooldown violation"
    ✅ "최근에 비슷한 주제의 글을 작성해서 잠시 사용할 수 없습니다"
"""
...
def humanize_error(e: BaseException) -> str:
    ...
    if isinstance(e, PermissionError):
        if "seed_cooldown" in msg or "cooldown" in msg:
            return (
                "🛑 최근에 비슷한 주제의 글을 작성하셔서 잠시 사용할 수 없어요. "
    ...
    if isinstance(e, KeyError):
        return (
            "🔍 선택한 키워드를 더 이상 찾을 수 없어요. "
            "‘새로고침’ 버튼을 눌러 풀을 다시 불러오세요."
        )
    ...
    if isinstance(e, ConnectionError):
        return "🌐 외부 서비스(구글 시트 등)에 연결할 수 없어요. 잠시 후 다시 시도해주세요."

    if isinstance(e, RuntimeError) and "gspread" in msg:
        return (
            "🔧 구글 시트 연동 패키지를 찾을 수 없어요. "
    ...
    # 마지막 안전망 — 기술 노출 최소화
    return (
        f"⚠️ 예상치 못한 문제가 발생했어요. ‘로그/상태’ 화면에서 자세한 내용을 확인하세요. "
        f"(유형: {name})"
    )
